...
<div class="wrapper">
  ...
  <table class="table table-hover">
	  ...
	  <tbody>
	    ...
	    <!-- ProductVO 클래스의 필드명으로 코딩했지만, 호출은 getter메서드가 사용됨. -->
	    <tr>
	      ...
	      <td>
          <p style="text-decoration: line-through;"><c:out value="${productVO.pdt_price }" /></p>
          <c:out value="${productVO.pdt_dis_price }" />원
        </td>	      
	      ...
	      <td>
			...
			<button type="button" name="btnSalesYN" data-pdt_num="${productVO.pdt_num }" class="btn btn-link">변경</button>
	      </td>
	      <td>
...
<script>
  $(document).ready(function() {
    ...
    $("#btnSearch").on("click", function(){
      ...
      //console.log($(this).parent().find("select[name='type']").val());
      if($(this).parent().find("select[name='type']").val() == ''){
        alert("검색 타입을 선택하세요");
        return;
      }
    ...
    $("button[name='btnProductEdit']").on("click", function(){
      //브라우저 뒤로가기에 의한 파라미터 중복 추가 부분을 방지하기 위한 제거 작업.
